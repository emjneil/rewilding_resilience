    ...
    def step(self):
        ...
        if self.reintroduction == True:
            ...
            if self.schedule.time >= 193 and ((self.schedule.time % 12) + 1) == 3:
                ...
                if abs(new_results.iloc[-1]["Grassland"]-new_results.iloc[-1]["Grassland"]) < 5 and abs(new_results.iloc[-1]["Thorny Scrub"]-new_results.iloc[-1]["Thorny Scrub"]) < 5 and abs(new_results.iloc[-1]["Woodland"]-new_results.iloc[-1]["Woodland"]) < 5 and abs(new_results.iloc[-1]["Roe deer"]-new_results.iloc[-1]["Roe deer"]) < 5:
                    self.running = False
