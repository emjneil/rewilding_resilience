    ...
    def step(self):
        ...
        if self.reintroduction == True:
            ...
            if self.schedule.time >= 193 and ((self.schedule.time % 12) + 1) == 3:
                ...
                print("old scrub:", results.iloc[five_years_ago]["Thorny Scrub"], "new scrub:", results.iloc[-1]["Thorny Scrub"], "difference:", abs(results.iloc[five_years_ago]["Thorny Scrub"]-results.iloc[-1]["Thorny Scrub"]))
                print("old roe:", results.iloc[five_years_ago]["Roe deer"], "new roe:", results.iloc[-1]["Roe deer"], "difference:", abs(results.iloc[-1]["Roe deer"]-results.iloc[five_years_ago]["Roe deer"]))


                if (abs(results.iloc[-1]["Grassland"]-results.iloc[five_years_ago]["Grassland"]) < 2) and (abs(results.iloc[-1]["Thorny Scrub"]-results.iloc[five_years_ago]["Thorny Scrub"]) < 2) and (abs(results.iloc[-1]["Woodland"]-results.iloc[five_years_ago]["Woodland"]) < 2) and (abs(results.iloc[-1]["Roe deer"]-results.iloc[five_years_ago]["Roe deer"]) < 2):                
