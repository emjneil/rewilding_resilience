    ...
    def step(self):
        ...
        if self.reintroduction == True:
            ...
            if self.schedule.time >= 193 and ((self.schedule.time % 12) + 1) == 3:
                ...
                results = self.datacollector.get_model_vars_dataframe() 
                five_years_ago = self.schedule.time - 60
                if (abs(results.iloc[-1]["Grassland"]-results.iloc[-1]["Grassland"]) < 5) and (abs(results.iloc[-1]["Thorny Scrub"]-results.iloc[-1]["Thorny Scrub"]) < 5) and (abs(results.iloc[-1]["Woodland"]-results.iloc[-1]["Woodland"]) < 5) and (abs(results.iloc[-1]["Roe deer"]-results.iloc[-1]["Roe deer"]) < 5):                
                
                # get the conditions of each year
                results =  self.datacollector.get_model_vars_dataframe()
                if first_year < self.schedule.time:                     
                    new_results = self.datacollector.get_model_vars_dataframe()

                    print("continuing", self.schedule.time)
                    print("old scrub:", results.iloc[-1]["Thorny Scrub"], "new old:", new_results.iloc[-1]["Thorny Scrub"], "difference:", abs(new_results.iloc[-1]["Thorny Scrub"]-results.iloc[-1]["Thorny Scrub"]))
                    print("old roe:", results.iloc[-1]["Roe deer"], "new roe:", new_results.iloc[-1]["Roe deer"], "difference:", abs(new_results.iloc[-1]["Roe deer"]-results.iloc[-1]["Roe deer"]))


                    if (abs(new_results.iloc[-1]["Grassland"]-results.iloc[-1]["Grassland"]) < 5) and (abs(new_results.iloc[-1]["Thorny Scrub"]-results.iloc[-1]["Thorny Scrub"]) < 5) and (abs(new_results.iloc[-1]["Woodland"]-results.iloc[-1]["Woodland"]) < 5) and (abs(new_results.iloc[-1]["Roe deer"]-results.iloc[-1]["Roe deer"]) < 5):
