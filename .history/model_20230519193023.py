    ...
    def step(self):
        ...
        if self.reintroduction == True:
            ...
            if self.schedule.time >= 193 and ((self.schedule.time % 12) + 1) == 3:
                ...
                # record the first year 
                first_year ==  
                previous_time = self.schedule.time
                results =  self.datacollector.get_model_vars_dataframe()
                if previous_time < self.schedule.time: 
                    new_results = self.datacollector.get_model_vars_dataframe()

                print("continuing", self.schedule.time)
                print("old grass:", results.iloc[-1]["Grassland"], "new grass:", new_results.iloc[-1]["Grassland"], "difference:", abs(new_results.iloc[-1]["Grassland"]-results.iloc[-1]["Grassland"]))
