...
class KneppModel(mesa.Model):
    ...
    def __init__(self, initial_roe, roe_deer_reproduce, roe_deer_gain_from_saplings, roe_deer_gain_from_trees, roe_deer_gain_from_scrub, roe_deer_gain_from_young_scrub, roe_deer_gain_from_grass,
                        ...
                        chance_scrub_saves_saplings, stocking_shift, 
        ...
        self.stocking_shift = stocking_shift



        # then add the herbivores as points
        for _ in range(initial_roe * self.stocking_shift): # number of roe deer
    ...
    def step(self):
        ...
        if self.reintroduction == True:
            ...
            if self.schedule.time == 49:
                self.add_herbivores(exmoor_pony_agent, int(23*self.stocking_shift), grazer_move) 
                self.add_herbivores(longhorn_cattle_agent, int(53*self.stocking_shift), grazer_move)
                number_sows = random.randint(4*self.stocking_shift,int(8*self.stocking_shift))
                number_piglets = int((20*self.stocking_shift)-number_sows)
            ...
            if self.schedule.time == 61:
                ...
                # exmoor ponies
                if outputs.iloc[61]['Exmoor pony'] < int(13**self.stocking_shift):
                    number_to_add = int(13*self.stocking_shift - outputs.iloc[61]['Exmoor pony'])
                    self.add_herbivores(exmoor_pony_agent, number_to_add, grazer_move)
                # longhorn cows
                if outputs.iloc[61]['Longhorn cattle'] >= int(77*self.stocking_shift):
                    number_to_subtract = int(-77*self.stocking_shift + outputs.iloc[61]['Longhorn cattle'])
                    self.remove_herbivores(longhorn_cattle_agent, number_to_subtract)
                else:
                    number_to_add = int(77*self.stocking_shift - outputs.iloc[61]['Longhorn cattle'])
                    self.add_herbivores(longhorn_cattle_agent, number_to_add, grazer_move)
                # add fallow deer
                self.add_herbivores(fallow_deer_agent, int(53*self.stocking_shift), mixed_diet_move)
                # tamworth pigs
                if outputs.iloc[61]['Tamworth pigs'] >= int(17*self.stocking_shift):
                    number_to_subtract = int(-17*self.stocking_shift + outputs.iloc[61]['Tamworth pigs'])
                    self.remove_pig(tamworth_pig_agent, number_to_subtract, 0, 0)
                else:
                    number_to_add = int(17*self.stocking_shift - outputs.iloc[61]['Tamworth pigs'])
                    self.add_pig(tamworth_pig_agent, number_to_add, 0, 0, random_move)
            # March 2011
            if self.schedule.time == 73:
                outputs = self.datacollector.get_model_vars_dataframe()
                # exmoor ponies
                if outputs.iloc[73]['Exmoor pony'] >= int(15**self.stocking_shift): 
                    number_to_subtract = int(-15*self.stocking_shift + outputs.iloc[73]['Exmoor pony'])
                    self.remove_herbivores(exmoor_pony_agent, number_to_subtract)
                else:
                    number_to_add = int(15*self.stocking_shift - outputs.iloc[73]['Exmoor pony'])
                    self.add_herbivores(exmoor_pony_agent, number_to_add, grazer_move)
                # longhorn cattle
                if outputs.iloc[73]['Longhorn cattle'] >= int(92*self.stocking_shift):
                    number_to_subtract = int(-92*self.stocking_shift + outputs.iloc[73]['Longhorn cattle'])
                    self.remove_herbivores(longhorn_cattle_agent, number_to_subtract)
                else:
                    number_to_add = int(92*self.stocking_shift - outputs.iloc[73]['Longhorn cattle'])
                    self.add_herbivores(longhorn_cattle_agent, number_to_add, grazer_move)
                # fallow deer
                if outputs.iloc[73]['Fallow deer'] >= int(81*self.stocking_shift):
                    number_to_subtract = int(-81*self.stocking_shift + outputs.iloc[73]['Fallow deer'])
                    self.remove_herbivores(fallow_deer_agent, number_to_subtract)
                else:
                    number_to_add = int(81*self.stocking_shift - outputs.iloc[73]['Fallow deer'])
                    self.add_herbivores(fallow_deer_agent, number_to_add, mixed_diet_move)
                # tamworth pigs
                if outputs.iloc[73]['Tamworth pigs'] >= int(22*self.stocking_shift):
                    number_to_subtract = int(-22*self.stocking_shift + outputs.iloc[73]['Tamworth pigs'])
                    self.remove_pig(tamworth_pig_agent, number_to_subtract, 0, 0)
                else:
                    number_to_add = int(22*self.stocking_shift - outputs.iloc[73]['Tamworth pigs'])
                    self.add_pig(tamworth_pig_agent, number_to_add, 0, 0, random_move)
            # March 2012
            if self.schedule.time == 85:
                outputs = self.datacollector.get_model_vars_dataframe()
                if outputs.iloc[85]['Exmoor pony'] >= int(17*self.stocking_shift): # randomly choose that many exmoor ponies and delete them
                    number_to_subtract = int(-17*self.stocking_shift + outputs.iloc[85]['Exmoor pony'])
                    self.remove_herbivores(exmoor_pony_agent, number_to_subtract)
                else:
                    number_to_add = int(17*self.stocking_shift - outputs.iloc[85]['Exmoor pony'])
                    self.add_herbivores(exmoor_pony_agent, number_to_add, grazer_move)
                if outputs.iloc[85]['Longhorn cattle'] >= int(16*self.stocking_shift):
                    number_to_subtract = int(-116*self.stocking_shift + outputs.iloc[85]['Longhorn cattle'])
                    self.remove_herbivores(longhorn_cattle_agent, number_to_subtract)
                else:
                    number_to_add = int(116*self.stocking_shift - outputs.iloc[85]['Longhorn cattle'])
                    self.add_herbivores(longhorn_cattle_agent, number_to_add, grazer_move)
                if outputs.iloc[85]['Fallow deer'] >= int(100*self.stocking_shift):
                    number_to_subtract = int(-100*self.stocking_shift + outputs.iloc[85]['Fallow deer'])
                    self.remove_herbivores(fallow_deer_agent, number_to_subtract)
                else:
                    number_to_add = int(100*self.stocking_shift - outputs.iloc[85]['Fallow deer'])
                    self.add_herbivores(fallow_deer_agent, number_to_add, mixed_diet_move)
                if outputs.iloc[85]['Tamworth pigs'] >= int(33*self.stocking_shift):
                    number_to_subtract = int(-33*self.stocking_shift + outputs.iloc[85]['Tamworth pigs'])
                    self.remove_pig(tamworth_pig_agent, number_to_subtract, 0, 0)
                else:
                    number_to_add = int(33*self.stocking_shift - outputs.iloc[85]['Tamworth pigs'])
                    self.add_pig(tamworth_pig_agent, number_to_add, 0, 0, random_move)
            # March 2013
            if self.schedule.time == 97:
                outputs = self.datacollector.get_model_vars_dataframe()
                # exmoor ponies
                if outputs.iloc[97]['Exmoor pony'] >= int(10*self.stocking_shift): # randomly choose that many exmoor ponies and delete them
                    number_to_subtract = int(-10*self.stocking_shift + outputs.iloc[97]['Exmoor pony'])
                    self.remove_herbivores(exmoor_pony_agent, number_to_subtract)
                else:
                    number_to_add = int(10*self.stocking_shift - outputs.iloc[97]['Exmoor pony'])
                    self.add_herbivores(exmoor_pony_agent, number_to_add, grazer_move)
                # red deer
                self.add_herbivores(red_deer_agent, int(13*self.stocking_shift), mixed_diet_move)
                # longhorn cattle
                if outputs.iloc[97]['Longhorn cattle'] >= int(129*self.stocking_shift):
                    number_to_subtract = int(-129*self.stocking_shift + outputs.iloc[97]['Longhorn cattle'])
                    self.remove_herbivores(longhorn_cattle_agent, number_to_subtract)
                else:
                    number_to_add = int(129*self.stocking_shift - outputs.iloc[97]['Longhorn cattle'])
                    self.add_herbivores(longhorn_cattle_agent, number_to_add, grazer_move)
                # fallow deer
                if outputs.iloc[97]['Fallow deer'] >= int(100*self.stocking_shift):
                    number_to_subtract = int(-100*self.stocking_shift + outputs.iloc[97]['Fallow deer'])
                    self.remove_herbivores(fallow_deer_agent, number_to_subtract)
                else:
                    number_to_add = int(100*self.stocking_shift - outputs.iloc[97]['Fallow deer'])
                    self.add_herbivores(fallow_deer_agent, number_to_add, mixed_diet_move)
                # tamworth pigs
                if outputs.iloc[97]['Tamworth pigs'] >= int(6*self.stocking_shift):
                    number_to_subtract = int(-6 + outputs.iloc[97]['Tamworth pigs'])
                    self.remove_pig(tamworth_pig_agent, number_to_subtract, 0, 0)
                else:
                    number_to_add = int(6*self.stocking_shift - outputs.iloc[97]['Tamworth pigs'])
                    self.add_pig(tamworth_pig_agent, number_to_add, 0, 0, random_move)
            # March 2014
            if self.schedule.time == 109:
                outputs = self.datacollector.get_model_vars_dataframe()
                if outputs.iloc[109]['Exmoor pony'] >= int(10*self.stocking_shift): # randomly choose that many exmoor ponies and delete them
                    number_to_subtract = int(-10*self.stocking_shift + outputs.iloc[109]['Exmoor pony'])
                    self.remove_herbivores(exmoor_pony_agent, number_to_subtract)
                else:
                    number_to_add = int(10*self.stocking_shift - outputs.iloc[109]['Exmoor pony'])
                    self.add_herbivores(exmoor_pony_agent, number_to_add, grazer_move)
                if outputs.iloc[109]['Longhorn cattle'] >= int(264*self.stocking_shift):
                    number_to_subtract = int(-264*self.stocking_shift + outputs.iloc[109]['Longhorn cattle'])
                    self.remove_herbivores(longhorn_cattle_agent, number_to_subtract)
                else:
                    number_to_add = int(264*self.stocking_shift - outputs.iloc[109]['Longhorn cattle'])
                    self.add_herbivores(longhorn_cattle_agent, number_to_add, grazer_move)
                if outputs.iloc[109]['Fallow deer'] >= int(100*self.stocking_shift):
                    number_to_subtract = int(-100*self.stocking_shift + outputs.iloc[109]['Fallow deer'])
                    self.remove_herbivores(fallow_deer_agent, number_to_subtract)
                else:
                    number_to_add = int(100*self.stocking_shift - outputs.iloc[109]['Fallow deer'])
                    self.add_herbivores(fallow_deer_agent, number_to_add, mixed_diet_move)
                if outputs.iloc[109]['Tamworth pigs'] >= int(18*self.stocking_shift):
                    number_to_subtract = int(-18*self.stocking_shift + outputs.iloc[109]['Tamworth pigs'])
                    self.remove_pig(tamworth_pig_agent, number_to_subtract, 0, 0)
                else:
                    number_to_add = int(18*self.stocking_shift - outputs.iloc[109]['Tamworth pigs'])
                    self.add_pig(tamworth_pig_agent, number_to_add, 0, 0, random_move)
                if outputs.iloc[109]['Red deer'] >= int(13*self.stocking_shift):
                    number_to_subtract = int(-13*self.stocking_shift + outputs.iloc[109]['Red deer'])
                    self.remove_herbivores(red_deer_agent, number_to_subtract)
                else:
                    number_to_add = int(13*self.stocking_shift - outputs.iloc[109]['Red deer'])
            ...
            if self.schedule.time == 119:
                self.add_pig(tamworth_pig_agent, 0, 0, int(1*self.stocking_shift), random_move)
            # Feb 2015 - assumed 1 boar removed, -2 cows
            if self.schedule.time == 120:
                self.remove_pig(tamworth_pig_agent, 0, 0, int(1**self.stocking_shift))
            # March 2015
            if self.schedule.time == 121:
                outputs = self.datacollector.get_model_vars_dataframe()
                #  exmoor ponies
                if outputs.iloc[121]['Exmoor pony'] >= int(10*self.stocking_shift):
                    number_to_subtract = int(-10*self.stocking_shift + outputs.iloc[121]['Exmoor pony'])
                    self.remove_herbivores(exmoor_pony_agent, number_to_subtract)
                else:
                    number_to_add = int(10*self.stocking_shift - outputs.iloc[121]['Exmoor pony'])
                    self.add_herbivores(exmoor_pony_agent, number_to_add, grazer_move)
                # longhorn cattle
                if outputs.iloc[121]['Longhorn cattle'] >= int(107*self.stocking_shift):
                    number_to_subtract = int(-107*self.stocking_shift + outputs.iloc[121]['Longhorn cattle'])
                    self.remove_herbivores(longhorn_cattle_agent, number_to_subtract)
                else:
                    number_to_add = int(107*self.stocking_shift - outputs.iloc[121]['Longhorn cattle'])
                    self.add_herbivores(longhorn_cattle_agent, number_to_add, grazer_move)
                # fallow deer
                if outputs.iloc[121]['Fallow deer'] >= int(100*self.stocking_shift):
                    number_to_subtract = int(-100*self.stocking_shift + outputs.iloc[121]['Fallow deer'])
                    self.remove_herbivores(fallow_deer_agent, number_to_subtract)
                else:
                    number_to_add = int(100*self.stocking_shift - outputs.iloc[121]['Fallow deer'])
                    self.add_herbivores(fallow_deer_agent, number_to_add, mixed_diet_move)
                # tamworth pigs
                ...
                if len(number_piglets) >= int(13*self.stocking_shift):
                    number_to_subtract_piglets = int(-13*self.stocking_shift) + len(number_piglets)
                    self.remove_pig(tamworth_pig_agent, number_to_subtract_piglets, 0, 0)
                else:
                    number_to_add_piglets = int(13*self.stocking_shift) - len(number_piglets)
                    self.add_pig(tamworth_pig_agent, number_to_add_piglets, 0, 0, random_move)
                if len(number_sows) >= int(5*self.stocking_shift):
                    number_to_subtract_sows = int(-5*self.stocking_shift) + len(number_sows)
                    self.remove_pig(tamworth_pig_agent, 0, number_to_subtract_sows, 0)
                else:
                    number_to_add_sows = int(5*self.stocking_shift) - len(number_sows)
                    self.add_pig(tamworth_pig_agent, 0, number_to_add_sows, 0, random_move)
                # red deer
                if outputs.iloc[121]['Red deer'] >= int(13*self.stocking_shift):
                    number_to_subtract = int(-13*self.stocking_shift + outputs.iloc[121]['Red deer'])
                    self.remove_herbivores(red_deer_agent, number_to_subtract)
                else:
                    number_to_add = int(13*self.stocking_shift - outputs.iloc[121]['Red deer'])
                    self.add_herbivores(red_deer_agent, number_to_add, mixed_diet_move)
            # April 2015
            if self.schedule.time == 122:
                self.remove_pig(tamworth_pig_agent, 0, int(1*self.stocking_shift), 0)
            # May 2015
            if self.schedule.time == 123:
                # May 2015: 8 piglets culled
                self.remove_pig(tamworth_pig_agent, int(8*self.stocking_shift), 0, 0)
            # June 2015: 5 cows culled
            if self.schedule.time == 124:
                self.remove_herbivores(longhorn_cattle_agent, int(5*self.stocking_shift))
            # August 2015: 2 fallow deer culled
            if self.schedule.time == 126:
                self.remove_herbivores(fallow_deer_agent, int(2*self.stocking_shift))
            # September 2015: 2 male fallow deer culled; 2 cattle culled and 3 bulls added
            if self.schedule.time == 128:
                self.remove_herbivores(fallow_deer_agent, int(2*self.stocking_shift))
                self.add_herbivores(longhorn_cattle_agent, int(1*self.stocking_shift), grazer_move)
            # Oct 2015: 2 female and 1 male fallow deer culled; 38 female cows and 1 bull removed
            if self.schedule.time == 128:
                self.remove_herbivores(fallow_deer_agent, int(3*self.stocking_shift))
                self.remove_herbivores(longhorn_cattle_agent, int(39*self.stocking_shift))
            # Nov 2015: -7 fallow deer, -1 piglet
            if self.schedule.time == 129:
                self.remove_herbivores(fallow_deer_agent, int(7*self.stocking_shift))                 
                self.remove_pig(tamworth_pig_agent, int(1*self.stocking_shift), 0, 0)       
            # Dec 2015: 6 fallow deer culled; 5 cows removed;
            if self.schedule.time == 130:
                self.remove_herbivores(fallow_deer_agent, int(6*self.stocking_shift))
                self.remove_herbivores(longhorn_cattle_agent, int(5*self.stocking_shift))
            # Jan 2016: 7 fallow deer culled; 4 pigs culled and 1 added
            if self.schedule.time == 131:
                self.remove_herbivores(fallow_deer_agent, int(7*self.stocking_shift))
                self.remove_pig(tamworth_pig_agent, int(4*self.stocking_shift), 0, 0)
                self.add_pig(tamworth_pig_agent, 0, 0, int(1*self.stocking_shift), random_move)
            # Feb 2016: 10 fallow deer culled; 2 pigs culled
            if self.schedule.time == 132:
                self.remove_herbivores(fallow_deer_agent, int(10*self.stocking_shift))
                self.remove_pig(tamworth_pig_agent, int(2*self.stocking_shift), 0, 0)
            ...
            if self.schedule.time == 133:
                self.add_herbivores(exmoor_pony_agent, int(1*self.stocking_shift), grazer_move)
                self.remove_pig(tamworth_pig_agent, int(1*self.stocking_shift), 0, int(1*self.stocking_shift))
                self.add_pig(tamworth_pig_agent, 0, int(3*self.stocking_shift), 0, random_move)
            # April 2016
            if self.schedule.time == 134:
                self.add_herbivores(longhorn_cattle_agent, int(1*self.stocking_shift), grazer_move)
            # May 2016
            if self.schedule.time == 135:
                self.remove_herbivores(longhorn_cattle_agent, int(2*self.stocking_shift))
            # June 2016
            if self.schedule.time == 136:
                self.remove_herbivores(longhorn_cattle_agent, int(26*self.stocking_shift))
            # July 2016
            if self.schedule.time == 137:
                self.remove_herbivores(longhorn_cattle_agent, int(2*self.stocking_shift))
            # August 2016
            if self.schedule.time == 138:
                self.remove_herbivores(fallow_deer_agent, int(5*self.stocking_shift))
            # September & Oct 2016
            if self.schedule.time == 139:
                self.add_herbivores(longhorn_cattle_agent, int(9*self.stocking_shift), grazer_move)
            # November 2016
            ...
            if self.schedule.time == 142:
                self.remove_herbivores(fallow_deer_agent, int(9*self.stocking_shift))
                self.remove_herbivores(longhorn_cattle_agent, int(13*self.stocking_shift))
                self.remove_pig(tamworth_pig_agent, int(4*self.stocking_shift), 0, 0)
            # January 2017
            if self.schedule.time == 143:
                self.remove_pig(tamworth_pig_agent, int(2*self.stocking_shift), int(2*self.stocking_shift), 0)
                self.add_pig(tamworth_pig_agent, 0, 0, int(1*self.stocking_shift), random_move)
            # February 2017
            if self.schedule.time == 144:
                self.remove_herbivores(fallow_deer_agent, int(8*self.stocking_shift))
                self.remove_pig(tamworth_pig_agent, int(1*self.stocking_shift), int(1*self.stocking_shift), int(1*self.stocking_shift))
            ...
            if self.schedule.time == 145:
                ...
                outputs = self.datacollector.get_model_vars_dataframe()
                if outputs.iloc[145]['Red deer'] >= int(14*self.stocking_shift):
                    number_to_subtract = int(-14*self.stocking_shift + outputs.iloc[145]['Red deer'])
                    self.remove_herbivores(red_deer_agent, number_to_subtract)
                else:
                    number_to_add = int(14*self.stocking_shift - outputs.iloc[145]['Red deer'])
                    self.add_herbivores(red_deer_agent, number_to_add, mixed_diet_move)
            # April 2017
            if self.schedule.time == 146:
                self.add_herbivores(longhorn_cattle_agent, int(3*self.stocking_shift), grazer_move)
            # June & July 2017
            if self.schedule.time == 148:
                self.remove_herbivores(longhorn_cattle_agent, int(21*self.stocking_shift))
            # August 2017
            if self.schedule.time == 150:
                self.remove_herbivores(fallow_deer_agent, int(16*self.stocking_shift))
            # September 2017
            ...
            if self.schedule.time == 152:
                self.remove_herbivores(fallow_deer_agent, int(4*self.stocking_shift))
                self.remove_herbivores(longhorn_cattle_agent, int(2*self.stocking_shift))
            # November 2017
            ...
            if self.schedule.time == 154:
                self.remove_herbivores(fallow_deer_agent, int(46*self.stocking_shift))
                self.remove_herbivores(red_deer_agent, int(1*self.stocking_shift))
                self.remove_pig(tamworth_pig_agent, int(4*self.stocking_shift), 0, 0)
            # January 2018
            if self.schedule.time == 155:
                self.remove_pig(tamworth_pig_agent, int(9*self.stocking_shift), 0, 0)
                self.add_pig(tamworth_pig_agent, 0, 0, int(1*self.stocking_shift), random_move)
            # February 2018
            if self.schedule.time == 156:
                self.remove_herbivores(fallow_deer_agent, int(14*self.stocking_shift))
                self.remove_herbivores(red_deer_agent, int(1*self.stocking_shift))
                self.remove_pig(tamworth_pig_agent, 0, 0, int(1*self.stocking_shift))
            ...
            if self.schedule.time == 157:
                self.remove_herbivores(exmoor_pony_agent, int(1*self.stocking_shift))
                output = self.datacollector.get_model_vars_dataframe()
                if output.iloc[157]['Fallow deer'] >= int(251*self.stocking_shift): 
                    number_to_subtract = int(-251*self.stocking_shift + output.iloc[157]['Fallow deer'])
                    self.remove_herbivores(fallow_deer_agent, number_to_subtract)
                else:
                    number_to_add = int(251*self.stocking_shift - output.iloc[157]['Fallow deer'])
                    self.add_herbivores(fallow_deer_agent, number_to_add,mixed_diet_move)
                self.remove_pig(tamworth_pig_agent, int(3*self.stocking_shift), 0, 0)
                self.add_pig(tamworth_pig_agent, 0, int(3*self.stocking_shift), 0, random_move)
            ...
            if self.schedule.time == 160:
                self.remove_herbivores(longhorn_cattle_agent, int(18*self.stocking_shift))
            # July 2018
            ...
            if self.schedule.time == 162:
                self.remove_herbivores(exmoor_pony_agent, int(9*self.stocking_shift))
                self.remove_herbivores(fallow_deer_agent, int(15*self.stocking_shift))
                self.remove_herbivores(longhorn_cattle_agent, int(1*self.stocking_shift))
                self.remove_herbivores(red_deer_agent, int(1*self.stocking_shift))
            # September 2018
            if self.schedule.time == 163:
                self.remove_herbivores(fallow_deer_agent, int(19*self.stocking_shift))
                self.add_herbivores(longhorn_cattle_agent, int(4*self.stocking_shift), grazer_move)
            # October 2018
