...
def create_params():
    ...
    for _ in range(number_simulations):
        ...
        param_ranges = pd.read_excel("ranges.xlsx", engine="openpyxl")
...
def run_model_long_term():
            ...
            if abs(gradients["Thorny Scrub"]) < 0.01 and abs(gradients["Grassland"]) < 0.01 and abs(gradients["Woodland"]) < 0.01:
                ...
                if (habs_2009['Grassland'].item() <= 89.9) and (habs_2009['Grassland'].item() >= 26.8) and (habs_2009['Thorny Scrub'].item() <= 51.8) and (habs_2009["Thorny Scrub"].item() >= 4.3) and (habs_2009["Woodland"].item() <= 17) and (habs_2009["Woodland"].item() >= 5.8) and (habs_2009["Roe deer"].item() <= 40) and (habs_2009["Roe deer"].item() >= 12):
                    ...
                    if (habs_2020['Grassland'].item() <= 36.8) and (habs_2020['Grassland'].item() >= 16.8) and (habs_2020['Thorny Scrub'].item() <= 61.8) and (habs_2020["Thorny Scrub"].item() >= 41.8) and (habs_2020["Woodland"].item() <= 31.5) and (habs_2020["Woodland"].item() >= 11.5) and (habs_2020["Roe deer"].item() <= 80) and (habs_2020["Roe deer"].item() >= 20): 
                        ...
                        failed_2020 += 1
                        print("failed 2020 filters:", (failed_2020/row["run_number"])*100,"%")
                else:
                    failed_2009 += 1
                    print("failed 2009 filters:", (failed_2009/row["run_number"])*100,"%")
    ...
    with open('failed_ecological_1.txt', 'w') as f:
        lines = ["failed roe deer:", str((failed_roe/row["run_number"])*100),'%', "failed equilibrium:", str((failed_equilibrium/row["run_number"])*100),"%", "failed 2009:", (failed_2009/row["run_number"])*100,"%", "failed 2020:", (failed_2020/row["run_number"])*100,"%"]
        f.writelines(lines)
    ...
    for index, row in my_time.iterrows():
        if (row["Roe deer"] <= 40) & (row["Roe deer"] >= 12) & (row["Grassland"] <= 89.9) & (row["Grassland"] >= 26.8) & (row["Woodland"] <= 17) & (row["Woodland"] >= 5.8) & (row["Thorny Scrub"] <= 51.8) & (row["Thorny Scrub"] >= 4.3):
...
def run_model_short():
    ...
    for index, row in final_parameters.iterrows():
        ...
        # keep this one in case any turn up < 184 timesteps bc of roe deer
        if end["Time"] < 184:
            failed_roe += 1
            print("failed roe:", (failed_roe/row["run_number"])*100,"%")
...
def check_ecological_criteria():

    print("run ecological criteria")

    # first select the parameters
    params = pd.read_csv("abm_passed_habs_params_test.csv").drop("Unnamed: 0", axis=1)
    ...
    while accepted_runs < 100:

        for index, row in params.iterrows():

            print(row["run_number"])
            print("number of accepted runs:", accepted_runs)
